# Switchboard AI MVP Foundation Implementation Plan
...
├── backend/
│   ├── pyproject.toml                      (switchboard-backend package)
│   ├── README.md
│   └── src/switchboard/
│       ├── __init__.py
│       ├── core/
...
└── docs/
    ...
    └── superpowers/plans/2026-05-09-switchboard-mvp-foundation.md   (this file)
```
...
**Files:**
- Create: `pyproject.toml` (root), `.python-version`, `backend/pyproject.toml`, `backend/src/switchboard/__init__.py`
...
**Files:**
- Create: `backend/src/switchboard/core/config.py`, `core/logging.py`, `core/ids.py`, `core/time.py`
- Create: `backend/src/switchboard/db/session.py`, `db/seed.py`
- Create: `backend/src/switchboard/app.py` (FastAPI factory with health route)
- Create: `backend/src/switchboard/api/routes_health.py`
...
- [ ] FastAPI factory `create_app()` configures CORS for `web/` dev origin, mounts routers, lifespan calls `init_db()` and `seed_dev_data()` if `SWITCHBOARD_ENV=dev`
...
**Files:**
- Create: `backend/src/switchboard/models/enums.py`, `models/firma.py`, `models/customer.py`, `models/call.py`, `models/job.py`, `models/escalation.py`
- Modify: `backend/src/switchboard/db/seed.py` (insert one demo firma)
...
**Files:**
- Create: `backend/src/switchboard/tools/schemas.py`, `tools/declarations.py`, `tools/handlers.py`, `tools/mock_data.py`
- Create: `backend/src/switchboard/services/customer_service.py`, `services/booking_service.py`, `services/escalation_service.py`, `services/notification_service.py`, `services/triage_service.py`, `services/rot_service.py`
...
**Files:**
- Create: `backend/src/switchboard/bridge/audio.py`, `bridge/gemini_session.py`, `bridge/system_prompt.py`, `bridge/usage_tracker.py`, `bridge/ws.py`
- Modify: `backend/src/switchboard/app.py` (mount bridge router)
...
**Files:**
- Create: `backend/src/switchboard/agents/post_call_summary.py`, `agents/runner.py`
- Modify: `backend/src/switchboard/bridge/ws.py` (call `runner.summarize_call(call_id)` on call end)
...
**Files:**
- Create: `backend/src/switchboard/api/deps.py`, `api/routes_calls.py`, `api/routes_customers.py`, `api/routes_firma.py`, `api/ws_inbox.py`
- Modify: `backend/src/switchboard/app.py` (mount routers, OpenAPI tags, generate `openapi.json`)
...
- Modify: `backend/src/switchboard/app.py` (export OpenAPI to `web/lib/openapi.json` via dev script)
