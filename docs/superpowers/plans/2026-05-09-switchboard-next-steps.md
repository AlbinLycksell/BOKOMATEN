# Switchboard AI — Next-Steps Implementation Plan

**Status:** Draft for review
**Last updated:** 2026-05-09
**Author:** Founding eng
**Predecessor plan:** [`2026-05-09-switchboard-mvp-foundation.md`](./2026-05-09-switchboard-mvp-foundation.md) (complete)
...
- **Deliverable:**
  - `backend/src/switchboard/integrations/elks_voice.py` — adapter that maps 46elks Voice Streaming framing (`{event:"start|sound|hangup"}`) into the bridge's normalized `{event,payload}` protocol.
  - `backend/src/switchboard/api/routes_elks.py` — webhook endpoint returning the `voice_start` JSON pointing at our `wss://…/ws/bridge/{firma_id}/{call_id}`.
...
- **Required input:**
  ...
  - GCP project id + service account with Vertex AI User role (`SWITCHBOARD_GCP_PROJECT`).
- **Depends on:** Step 1.
- **Deliverable:**
  - `backend/src/switchboard/bridge/gemini_session.py` updated to support Vertex AI (currently uses `v1beta` API key path).
...
- **Deliverable:**
  ...
  - `backend/src/switchboard/core/auth.py` — JWKS verification, replaces `TenantMiddleware` dev path with header-injection from a verified JWT.
...
- **Deliverable:**
  ...
  - `backend/src/switchboard/db/postgres.py` — connection pool, low-privilege role, `SET app.firma_id` per-checkout hook.
...
- **Required input:**
  - GCP project + billing.
  - Decision: per-firma bucket creation timing. Options: lazy (on first call), eager (on firma onboarding). Recommendation: eager — onboarding step creates `gs://switchboard-rec-{firma_id}-eu` with CMEK from `projects/{p}/locations/europe-west4/keyRings/switchboard/cryptoKeys/firma-{id}`.
...
- **Deliverable:**
  - `backend/src/switchboard/services/recording_service.py` — buffers audio per call, writes MP3 to GCS on call end, generates signed URL on demand.
  - `backend/src/switchboard/integrations/gcs.py` — bucket lifecycle, CMEK, signed URLs.
...
- **Required input:**
  - 46elks SMS plan + sender id (`SWITCHBOARD` sender or per-firma alias). 46elks supports custom sender ids with verification.
...
- **Deliverable:**
  - `backend/src/switchboard/integrations/elks_sms.py` — POST to `https://api.46elks.com/a1/SMS`.
...
- **Required input:**
  ...
  - OAuth client_id + secret (`SWITCHBOARD_FORTNOX_CLIENT_ID`, `_SECRET`).
...
- **Deliverable:**
  - `backend/src/switchboard/integrations/fortnox.py` — typed client wrapping the relevant endpoints.
...
- **Required input:**
  ...
  - Domain: `app.switchboard.se` for backend, `bridge.switchboard.se` for bridge. Founders to register if not done.
...
- **Required input:**
  ...
  - Decision: low-privilege application role name + grant template. Recommendation: `switchboard_app` role, owner of nothing, `GRANT SELECT, INSERT, UPDATE, DELETE` on tenant-scoped tables, no `BYPASSRLS`.
...
- **Deliverable:**
  - `backend/src/switchboard/eval/` — dataset loaders, replay harness, metrics, report.
...
- **Required input:**
  - Decision: voice library — single Switchboard voice or per-firma choice (PRD §13 open question). Recommendation: 3 default options (warm female, neutral male, energetic young) at launch.
...
- **Depends on:** Step 5.
- **Deliverable:**
  - `backend/src/switchboard/services/redaction_service.py`.
...
- **Deliverable:**
  - `backend/src/switchboard/integrations/hantverksdata.py`.
...
| Domain: `app.switchboard.se`, `bridge.switchboard.se` | 8 | Founders |
