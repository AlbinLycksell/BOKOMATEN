# Switchboard AI — Launch Readiness Plan

**Status:** Draft for review
**Last updated:** 2026-05-09
**Predecessor plans:**
- [`2026-05-09-switchboard-mvp-foundation.md`](./2026-05-09-switchboard-mvp-foundation.md) — complete
- [`2026-05-09-switchboard-next-steps.md`](./2026-05-09-switchboard-next-steps.md) — partly complete; this plan supersedes the open items
...
- **Goal:** `https://app.switchboard.se/health` and `https://bridge.switchboard.se/health` return 200 from real Cloud Run services.
- **Why now:** Every other tier-A item depends on a live deploy.
- **Required input:**
  - **GCP project id** (recommend: `switchboard-prod`). Founders create at https://console.cloud.google.com/projectcreate. ETA 5 min.
  - **GCP billing account id** linked to the project.
  - **Domain access** for `switchboard.se` to add DNS records for `app` and `bridge` subdomains.
  ...
  - **Sentry account** (EU region) — create projects `switchboard-app`, `switchboard-bridge`, `switchboard-web`. ETA 10 min.
...
- **Acceptance:**
  ...
  - `gcloud sql connect switchboard-pg --user=switchboard_app` succeeds.
  - `alembic upgrade head` recorded in container logs on first cold-start.
  - Per-call structured logs visible via `gcloud logging tail` filter `resource.labels.service_name="switchboard-bridge"`.
...
- **Required input:**
  ...
  - **46elks Voice URL** configured: `https://app.switchboard.se/api/integrations/elks/voice/inbound`.
...
- **Deliverable:**
  ...
  - First call recording stored in `gs://switchboard-rec-{firma}-europe-west4/calls/{call_id}.mp3`.
...
- **Required input:**
  ...
  - **Storage:** `gs://switchboard-eval-recordings/v1/` (private GCS bucket, IAM-restricted).
- **Depends on:** A2.
- **Deliverable:**
...
- **Depends on:** A1.
- **Deliverable:**
  - `backend/src/switchboard/scripts/erase_tenant.py` — single-transaction SQL DELETE in topo order, then `GCSStorage.delete_tenant`, then KMS key version destroy, then audit log entry.
...
- **Required input:**
  ...
  - **Eskaleringskedja default text.** Recommendation in [`docs/superpowers/plans/2026-05-09-switchboard-input-form.md`](./2026-05-09-switchboard-input-form.md).
- **Depends on:** A4.
- **Deliverable:**
...
- **Depends on:** A4, B2.
- **Deliverable:**
  - `backend/src/switchboard/integrations/fortnox.py` — typed client + OAuth callback at `/api/integrations/fortnox/callback`.
...
- **Required input:**
  - **500 labeled Swedish calls** (intent + severity + expected actions).
    ...
    - Storage: `gs://switchboard-eval-dataset/v1/`.
  - **Slack webhook URL** for `#switchboard-eval` weekly digest.
...
- **Deliverable:**
  - `backend/src/switchboard/eval/` — dataset loaders, replay harness, metrics, HTML report.
...
- **Goal:** Every pilot firma's SMS appears from their firma name (not "Switchboard").
...
- **Deliverable:**
  ...
  - Falls back to platform `Switchboard` until verification completes.
...
- **Depends on:** A2.
- **Deliverable:**
  - `backend/src/switchboard/services/pii_audit_service.py` — runs in a Cloud Tasks queue after each call.
...
| Domain DNS access (`switchboard.se`) | Founders | already owned |
...
| Slack webhook for `#switchboard-eval` | Slack admin | 5 min |
...
- When all Tier-A items are done, update [`docs/superpowers/plans/2026-05-09-switchboard-input-form.md`](./2026-05-09-switchboard-input-form.md) and mark the corresponding rows green.
