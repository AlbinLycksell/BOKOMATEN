# Svarsa AI — Input Form

**Companion to:** [`2026-05-09-svarsa-next-steps.md`](./2026-05-09-svarsa-next-steps.md)
...
- [ ] **46elks API username + password**
  ...
  - ETA: 1 minute
  - Store in 1Password as `Svarsa / 46elks API`. Reference here, not the value.
...
- [ ] **GCP project**
  - How to create: https://console.cloud.google.com/projectcreate — name `svarsa-prod` (or `svarsa-staging`), org `siftlab.com` if you want it under that org.
...
- [ ] **Service account for backend**
  - How: GCP console → IAM & Admin → Service Accounts → Create. Name `svarsa-backend`. Grant roles: `Vertex AI User`, `Cloud Run Invoker`, `Secret Manager Secret Accessor`, `Cloud SQL Client`, `Storage Object Admin`.
  - ETA: 10 minutes
...
- [ ] **10 representative recorded Swedish caller utterances**
  ...
  - Storage: drop into a private GCS bucket `gs://svarsa-eval-recordings/` (or share via Google Drive while waiting for GCS).
...
- [ ] **Google OAuth client**
  - How to create: GCP console → APIs & Services → Credentials → Create Credentials → OAuth client ID → Application type: Web → Authorized redirect URI: `https://app.svarsa.se/api/auth/callback/google` (and `http://localhost:3000/api/auth/callback/google` for dev). First time you'll be asked to set up the OAuth consent screen — pick "External", scopes "openid email profile".
  - ETA: 15 minutes
...
- [ ] **Database + low-privilege role**
  - How: Connect via Cloud SQL Proxy locally, run:
    ```sql
    CREATE DATABASE svarsa;
    CREATE ROLE svarsa_app LOGIN PASSWORD '<random>';
    GRANT CONNECT ON DATABASE svarsa TO svarsa_app;
    GRANT USAGE ON SCHEMA public TO svarsa_app;
...
- [ ] **Cloud KMS keyring**
  - How to create: GCP console → KMS → Create Keyring `svarsa` in `europe-west4`. CryptoKey per firma is created at onboarding by the recording service.
  - ETA: 2 minutes
...
- [ ] **SMS sender id**
  ...
  - **B. Platform `SVARSA` sender id**
    - Pros: One verification, instant for new firmor
    - Cons: Recipient sees "SVARSA" not the firma's name; weaker conversion
...
- [ ] **Fortnox OAuth application**
  - How: Developer portal → Create application. Provide: name "Svarsa AI", redirect URL `https://app.svarsa.se/api/integrations/fortnox/callback`, scopes: `customer`, `bookkeeping`, `invoice` (read), and `connectfile` (for attachments). Skip `journalvouchers` for v1.
  - ETA: 5 minutes
  - **Client ID:** ______________________________________
  - **Client secret (1Password ref):** _________________
...
- [ ] **Domain ownership**
  ...
  - **`app.svarsa.se`** (Application Backend) — owned by: ____
  - **`bridge.svarsa.se`** (Realtime Bridge) — owned by: ____
  - **Recommendation:** register `svarsa.se` if not done; we'll add subdomain routing via Cloud Run domain mapping.
  - **Registrar:** _______________________________________
  - **DNS provider:** ____________________________________

- [ ] **GitHub Actions Workload Identity Federation**
  - How: GCP console → IAM & Admin → Workload Identity Federation → Create pool → Add provider for GitHub OIDC. Grant the `svarsa-backend` SA `Workload Identity User` role on the pool. (Saves dealing with JSON keys.)
...
- [ ] **Artifact Registry repository**
  - How: GCP console → Artifact Registry → Create Repository → Format Docker → Region `europe-west4` → Name `svarsa`.
  - ETA: 1 minute
  - **Repository:** `europe-west4-docker.pkg.dev/<project>/svarsa`
...
- [ ] **Application role name**
  - **Recommendation:** `svarsa_app` (no `BYPASSRLS`, no `SUPERUSER`, only `LOGIN` + tenant-scoped grants)
  - **Choice:** [x] svarsa_app   [ ] other: _______
...
- [ ] **500 labeled Swedish calls**
  ...
  - Storage: `gs://svarsa-eval-dataset/v1/` (private, IAM restricted).
...
- [ ] **Slack webhook for weekly eval reports**
  - How: Create a channel `#svarsa-eval` → Apps → Incoming Webhooks → Add → copy URL.
  - ETA: 5 minutes
...
- [ ] **Greeting wizard intro line in Swedish**
  - Suggested: `Vi hjälper dig sätta upp Svarsa AI på 30 minuter. Klart börjar AI:n svara dina samtal redan i kväll.`
  - **Final:** _________________________________________
...
- [ ] **Critical alerts entitlement (Apple)** — for emergency push that bypasses Do Not Disturb
  - How: Apple Developer Portal → Certificates → Identifiers → request the entitlement. Apple reviews; ~2–4 weeks. Include a 1-page justification ("Svarsa is a paid receptionist tool; an emergency push that bypasses DND is the core value proposition").
  - **Submitted on:** ________________
  - **Approved on:** ________________
...
- [ ] **Sentry EU project**
  - How: https://sentry.io → Create org → Region: EU → Create projects `svarsa-backend`, `svarsa-bridge`, `svarsa-web`.
